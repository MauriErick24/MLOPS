"""Visualizaciones exploratorias y de evaluación."""
...
class FraudVisualizer:
    ...
    def plot_overview(self, dataframe: pd.DataFrame) -> None:
        ...
        axes[0, 0].bar(
            ["Normal", "Fraude"], [stats["normal"], stats["frauds"]], color=colors
        )
        ...
        sns.boxplot(
            x="Class", y="Amount", data=box_data, ax=axes[0, 1], palette=colors
        )
        ...
        for class_id, label, color in [
            ...
            axes[1, 0].hist(
                hours, bins=48, alpha=0.6, label=label, color=color, density=True
            )
        axes[1, 0].legend()
    ...
    def plot_model_comparison(y_true, scores: dict[str, np.ndarray], results: dict) -> None:
        ...
        for name, probabilities in scores.items():
            ...
            axes[0].plot(
                fpr, tpr, label=f"{name} ({roc_auc_score(y_true, probabilities):.4f})"
            )
            axes[1].plot(
                recall,
                precision,
